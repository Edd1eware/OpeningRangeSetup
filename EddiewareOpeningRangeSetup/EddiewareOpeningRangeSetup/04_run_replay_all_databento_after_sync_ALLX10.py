"""
Runner: replay de las ~481 fechas DataBento en X10 (sin fase X1).

Todas las fechas corren directamente en X10 en una sola fase.

Uso:
  python 04_run_replay_all_databento_after_sync_ALLX10.py
  python 04_run_replay_all_databento_after_sync_ALLX10.py --prepare-only
  python 04_run_replay_all_databento_after_sync_ALLX10.py --force
  python 04_run_replay_all_databento_after_sync_ALLX10.py --step
  python 04_run_replay_all_databento_after_sync_ALLX10.py --compare-only
"""
...
def _count_trades_in_window(last_n: int) -> tuple[int, int]:
    all_csvs: list[Path] = []
    if ALLX10_DIR.exists():
        all_csvs.extend(ALLX10_DIR.glob("score_trade_result_*_NY.csv"))
...
def _check_trade_frequency(processed: int) -> tuple[bool, int, int]:
    if processed < TRADE_WINDOW:
        return False, 0, 0
    trades, window_size = _count_trades_in_window(TRADE_WINDOW)
...
def _run_batched(dates: list[str], run_plan, args, global_total: int) -> tuple[bool, list, bool, int]:
    failures_all  = []
    passed_all    = True
    processed     = 0
    stage_done    = 0
    ...
    for tz_offset, batch in batches:
        ...
        passed, failures = replay_sync.run_replay_period(
            batch,
            output_folder=ALLX10_DIR,
            run_plan=run_plan,
            report_prefix="databento_allx10",
            force=args.force,
            step=args.step,
            ...
            progress_meta={
                "stage_index": 1,
                "stage_total": 1,
                "stage_label": "DataBento X10 ALL",
                "stage_period": f"{batch_start} -> {batch_end}",
                "global_target": global_total,
                "session_roots": [ALLX10_DIR],
                "run_label": "DataBento ALL-X10",
        ...
        print(
            f"  [X10 ALL] {stage_done}/{stage_total_n} ({pct:.0f}%) | "
            f"avg {avg_spd:.0f}s/fecha | ETA: {_fmt_eta(eta_stage_s)} | "
        ...
        if any(pct_prev < m <= pct for m in (25, 50, 75)):
            ...
            telegram.send_text(
                RESULTS_BASE,
                f"EW ORB NQ | DataBento X10 ALL — {pct:.0f}%\n"
...
def _save_equity_summary(n_dates: int) -> None:
    ...
    telegram.send_photo(
        RESULTS_BASE,
        str(dest_png),
        caption=f"EW ORB NQ | Equity final ALL-X10 — {n_dates} fechas{eq_str}",
    )
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Replay de ~481 fechas DataBento en X10 (sin fase X1)."
    )
...
def main():
    ...
    all_dates    = [d for d in all_dates if date.fromisoformat(d) <= last_allowed]

    print(f"\nDATAS DATABENTO ALL-X10: {len(all_dates)} fechas")
    print(f"  Rango: {all_dates[0]} -> {all_dates[-1]}")
    print(f"  Ultima permitida: {last_allowed}")
    print(f"  Output: {ALLX10_DIR}")
    ...
    eta_x10 = math.ceil(len(all_dates) * replay_sync.X10_TIMEOUT_SECONDS / 60)
    print(f"\n  ETA maximo: ~{eta_x10//60}h {eta_x10%60}m")

    if not args.compare_only:
        telegram.clear_telegram_before_run(RESULTS_BASE)

    ALLX10_DIR.mkdir(parents=True, exist_ok=True)
    print(f"\n=== X10 ALL: {len(all_dates)} fechas ===")

    with _SleepBlocker():
        passed, failures, stopped, total_processed = _run_batched(
            all_dates, run_plan_x10, args, global_total=len(all_dates)
        )

    print(f"Corrida {'STOP' if stopped else ('PASS' if passed else 'FAIL')} | {len(failures)} errores")
    telegram.check_and_notify_challenge(RESULTS_BASE, PNL_LOG, CHALLENGE_TARGET)

    if not stopped:
        telegram.send_equity_chart(
            RESULTS_BASE, PNL_LOG, target=CHALLENGE_TARGET,
            caption=f"ALL-X10 completado — {total_processed} fechas",
        )

    _report_end(failures, ALLX10_DIR)
    _save_equity_summary(total_processed)
    return 0 if (passed and not stopped) else 1
