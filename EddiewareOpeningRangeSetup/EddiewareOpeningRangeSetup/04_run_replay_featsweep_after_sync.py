...
def _get_replay_controls_by_handle():
    """Reemplazo de replay_sync.get_replay_controls: envuelve la ventana Replay por
    HANDLE (win32) en vez de enumerar TODO el desktop UIA (Desktop.windows), que se
    cuelga cuando hay apps pesadas abiertas (Chrome/Edge/webview2/Electron). La
    extraccion de controles (FROM/TO, Play, Stop) es identica a la del comun."""
    import win32gui
    ...
    def _cb(hwnd, _):
        if win32gui.IsWindowVisible(hwnd) and \
                win32gui.GetWindowText(hwnd).lower().startswith("replay"):
            hwnds.append(hwnd)
    ...
    hwnd = hwnds[0]
    replay = Application(backend="uia").connect(handle=hwnd).window(handle=hwnd)
    replay.set_focus()

    edits = replay.descendants(control_type="Edit")
    buttons = replay.descendants(control_type="Button")

    date_edits = [e for e in edits if e.element_info.class_name == "DateEdit"]
    from_box = next(
        (e for e in date_edits if e.element_info.name.strip().lower().startswith("from")),
        None,
    )
    to_box = next(
        (e for e in date_edits if e.element_info.name.strip().lower().startswith("to")),
        None,
    )

    start_button = next(
        (b for b in buttons if b.element_info.automation_id == "PlayButton"),
        None,
    )
    if start_button is None:
        start_button = next(
            (b for b in buttons if b.window_text().strip().lower() in {"start", "play", "iniciar"}),
            None,
        )

    stop_button = next(
        (b for b in buttons if b.window_text().strip().lower() in {"stop", "detener"}),
        None,
    )
    if stop_button is None and start_button is not None:
        ...
        if stop_candidates:
            stop_button = min(stop_candidates, key=lambda b: b.rectangle().left)

    if from_box is None or to_box is None:
        raise RuntimeError("No encontre los campos FROM/TO de Replay.")
    if start_button is None:
        raise RuntimeError("No encontre el boton Play de Replay.")
...
def main():
    ...
    # FIX cuelgue: sustituye la adquisicion de la ventana Replay por una via HANDLE
    # (rapida) en vez de enumerar todo el desktop UIA. Afecta run_one_date y
    # set_replay_speed porque el comun resuelve el nombre en su namespace al llamar.
    replay_sync.get_replay_controls = _get_replay_controls_by_handle
    print("FIX ventana Replay por HANDLE activo (evita cuelgue de Desktop.windows UIA).")
    ...
    if not args.compare_only:
        ...
        # Reinicia el balance corrido de Telegram: al arrancar la primera fecha de
        # la lista, borra el JSON para que el exporter recomience en $150,000 y
        # acumule SOLO las fechas de esta corrida (evita arrastrar totales viejos).
        balance_file = os.path.join(RESULTS_FOLDER, "telegram_balance.json")
        try:
            if os.path.exists(balance_file):
                os.remove(balance_file)
                print("Balance Telegram reiniciado a $150,000 (json borrado).")
        except OSError as exc:
            print(f"WARNING: no pude reiniciar el balance Telegram: {exc}")
