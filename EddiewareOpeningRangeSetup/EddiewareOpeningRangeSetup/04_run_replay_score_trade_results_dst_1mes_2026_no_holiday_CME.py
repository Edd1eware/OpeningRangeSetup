...
POLL_SECONDS = 1
REPLAY_SPEED_FACTOR = 10
NO_TRADE_CUTOFF_SECONDS = int((10 * 60 + 15) / REPLAY_SPEED_FACTOR) + 15  # ventana 09:30-09:40 a X10 + margen
MAX_WAIT_SECONDS = int((20 * 60) / REPLAY_SPEED_FACTOR) + 30  # ventana 09:30-09:50 a X10 + margen

# Deteccion de reproduccion (feriados no listados en HOLIDAYS_TO_SKIP no avanzan el replay).
MAX_START_ATTEMPTS = 3
DETECT_WINDOW_SECONDS = 3
...
def is_replay_running():
    _, _, _, _, stop_button = get_controls()
    return stop_button is not None and stop_button.is_enabled()


def start_replay_with_retries(date_label):
    for attempt in range(1, MAX_START_ATTEMPTS + 1):
        print(f"Intento {attempt}/{MAX_START_ATTEMPTS} de iniciar replay para {date_label}...")
        ...
        time.sleep(DETECT_WINDOW_SECONDS)

        if is_replay_running():
            print("Reproduccion detectada.")
            return started_at

        print(f"No se detecto reproduccion (posible feriado fuera de calendario CME). Esperando {RETRY_WAIT_SECONDS}s antes de reintentar...")
        time.sleep(RETRY_WAIT_SECONDS)

    print(f"{MAX_START_ATTEMPTS} intentos fallidos: dia sin datos de mercado. Saltando fecha.")
    return None
...
try:
    ...
    for date in DATES_DST:
        if date in HOLIDAYS_TO_SKIP:
            print("\n" + "=" * 70)
            print(f"SALTANDO FERIADO CME: {date}")
            print("=" * 70)
            continue
        ...
        started_at = start_replay_with_retries(date)

        if started_at is None:
            print(f"SALTANDO {date}: sin reproduccion tras {MAX_START_ATTEMPTS} intentos (feriado no listado en CME).")
            continue
