...
def send_overall_progress(
    ...
    *,
    ...
    """Progreso GLOBAL del recorrido (X/N fechas) con barra, % y ETA restante real.
    Lo llama el runner por fecha: como cada run_replay_period corre UNA sola fecha,
    el ETA por-etapa interno no sirve para el total; este mide sobre el recorrido
    completo (transcurrido / fechas hechas x fechas restantes)."""
    credentials = _read_credentials(results_folder)
    if credentials is None:
        return False
    ...
    lines = [
        f"EW Opening Range | Progreso recorrido ({run_label})",
        f"[{bar}] {pct * 100:.0f}%  ({done}/{total})",
        f"Ultima fecha: {date}",
    ]
    if pnl_usd is not None:
        ...
        if wins is not None and losses is not None:
            ...
            lines.append(f"Trades: {wins}W-{losses}L  WR {wr}")
    lines.append(
        f"Transcurrido: {_format_elapsed(elapsed_seconds)} | "
        f"Restante: {_format_eta(eta_seconds)}"
    )
    ...
    try:
        ...
        print(f"WARNING: no pude enviar progreso global Telegram: {exc}")
        return False
