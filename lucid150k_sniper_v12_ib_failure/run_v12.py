"""IB30 accepted breakout failure fade for V12."""
...
OUT = BASE / "output"
PREREG = BASE / "PREREGISTRO_LUCID150K_SNIPER_V12.md"
PREREG_SHA = "7a5edf16411bfbfc0d2bc3aee3796aa3b42f0f4b13e0053201954694a01e4576"
FAMILY = "IB30_ACCEPTED_FAILURE_FADE"
SEED = 0xB38D50C2741EA69F
BOOTSTRAPS = 10_000
...
def process_date(date: str) -> tuple[dict | None, str | None, str]:
    try:
        ...
        accept_cutoff = common.local_ts(date, "11:00:00")
        entry_cutoff = common.local_ts(date, "11:05:00")
        ...
        high, low = opening
        scan = nq.loc[
            (nq.index >= ib_end) & (nq.index <= accept_cutoff)
        ]
        previous_sign = 0
        previous_time = None
        count = 0
        ...
        for timestamp, bar in scan.iterrows():
            ...
            if count >= 2:
                ...
                break_sign = sign
                break
        if accepted_time is None or sequence_start is None:
            return None, None, "no_acceptance"

        pre = nq.loc[
            (nq.index >= sequence_start) & (nq.index <= accepted_time)
        ]
        extreme = (
            float(pre["high"].max())
            if break_sign == 1
        ...
        for timestamp, bar in window.iterrows():
            ...
            reclaimed = (
                float(bar["close"]) < high
                if break_sign == 1
                else float(bar["close"]) > low
            )
            if reclaimed:
...
def main() -> int:
    ...
    if dev_pass:
        ...
        pseudo_gates = {
            "V1_n_ge_40": bool(metrics.get("n", 0) >= 40),
    ...
    result = {
        "study": "LUCID150K-SNIPER-V12-IB-FAILURE",
