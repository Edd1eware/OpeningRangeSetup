"""Free Databento sample quote for bounded NQ+ES overnight context."""
...
OUT = BASE / "QUOTE_NQ_ES_OVERNIGHT_1M.json"
...
def main() -> int:
    ...
    for index, date in enumerate(sample, start=1):
        ...
        print(f"[{index:02d}/{len(sample)}] {date} cost=${cost:.6f}", flush=True)
        if index < len(sample):
            time.sleep(3)
    ...
    result = {
        ...
        "symbols": SYMBOLS,
        "schema": SCHEMA,
        "window_ny": "previous 18:00:00..current 09:30:00",
